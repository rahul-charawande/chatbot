...
if submitted and question:
    ...
    with st.spinner("Generating secure response..."):
        answer = generate_answer(context, question)
        # Mock response structure - in reality you'd get this from your API
        response = {
            "answer": answer,
            "sources": ["document.pdf"],
            "supporting_content": [
                {"text": chunk[:200] + "..." if len(chunk) > 200 else chunk, "source": "document.pdf"} 
    ...
    st.subheader("📄 Supporting Content")
    for content in response["supporting_content"]:
        with st.expander(f"From {content['source']}"):
            st.write(content["text"])
            st.caption(f"Source: {content['source']}")
